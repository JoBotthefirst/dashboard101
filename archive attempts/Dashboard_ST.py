...
view = st.sidebar.radio("Select view", ["Client ID Filter", "ClinicDesc Filter", "Activity Leaderboard", "SPC Chart", "Geographic Hotspot", "Referral ID Distribution", "Pareto Analysis", "Appointments by Day of Week", "Age Cluster Map"])
...
if view == "Client ID Filter":
    ...
    # Sidebar for HCP filter
    st.sidebar.header("HCP Filter")
    hcp_names_list = list(set(input_dataset["HCPName"]))
    selected_hcps = st.sidebar.multiselect("Select HCPs", options=hcp_names_list, help="Select healthcare providers to view appointments by day of week")
    ...
    day_counts = filtered_dataset['Day of Week'].value_counts().reindex(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']).reset_index()
    day_counts.columns = ['Day of Week', 'Appointment Count']

    # Plotting combined for service
    fig5 = px.bar(day_counts, x='Day of Week', y='Appointment Count', title="Appointments by Day of Week (Combined for Service)")
    ...
    fig5.update_layout(
        ...
        yaxis_title="Appointment Count",
        title={
            'text': "Appointments by Day of Week (Combined for Service)",
            'y': 0.9,
            'x': 0.5,
            'xanchor': 'center',
            'yanchor': 'top'
    ...
    if selected_hcps:
        ...
        hcp_filtered_dataset = filtered_dataset[filtered_dataset["HCPName"].isin(selected_hcps)]

        # Group by day of the week and HCP, and count the number of appointments
        hcp_day_counts = hcp_filtered_dataset.groupby(['Day of Week', 'HCPName']).size().unstack().reindex(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']).fillna(0)

        # Plotting for selected HCPs
        fig6 = px.bar(hcp_day_counts, x=hcp_day_counts.index, y=hcp_day_counts.columns, title="Appointments by Day of Week for Selected HCPs", barmode='group')
        ...
        fig6.update_layout(
            ...
            yaxis_title="Appointment Count",
            title={
                'text': "Appointments by Day of Week for Selected HCPs",
                'y': 0.9,
                'x': 0.5,
                'xanchor': 'center',
                'yanchor': 'top'
...
#     # Calculate the total number of appointments for each ClientID
#     client_appointments = input_dataset.groupby('ClientID')['TotalCountOfEventPerReferralID'].sum().reset_index()
    
#     # Sort the ClientIDs by the total number of appointments in descending order
#     client_appointments = client_appointments.sort_values(by='TotalCountOfEventPerReferralID', ascending=False)
    
#     # Calculate the cumulative percentage of appointments
#     client_appointments['CumulativeCount'] = client_appointments['TotalCountOfEventPerReferralID'].cumsum()
#     client_appointments['CumulativePercentage'] = 100 * client_appointments['CumulativeCount'] / client_appointments['TotalCountOfEventPerReferralID'].sum()
    
#     # Find the Client IDs that make up the top 20% of activity count
#     top_20_clients = client_appointments[client_appointments['CumulativePercentage'] <= 20]
    
#     # Create a Pareto chart
#     fig7 = px.bar(client_appointments, x=client_appointments.index + 1, y='TotalCountOfEventPerReferralID', title='Pareto Analysis of Client ID Activity')
    
#     # Add a line for the cumulative percentage
#     fig7.add_trace(go.Scatter(
#         x=client_appointments.index + 1,
#         y=client_appointments['CumulativePercentage'],
#         mode='lines+markers',
#         name='Cumulative Percentage',
#         yaxis='y2'
#     ))
    
#     # Add a horizontal line for the 20% threshold
#     fig7.add_shape(
#         type='line',
#         x0=0,
#         y0=20,
#         x1=len(client_appointments),
#         y1=20,
...
#         ),
#         title={
#             'text': "Pareto Analysis of Client ID Activity",
#             'y': 0.9,
#             'x': 0.5,
#             'xanchor': 'center',
#             'yanchor': 'top'
...
#     # Add hover information for top 20% clients
#     fig7.update_traces(
#         hovertemplate='<b>Client ID:</b> %{customdata[0]}<br><b>Appointments:</b> %{y}<br><b>Cumulative Percentage:</b> %{customdata[1]:.2f}%',
#         customdata=np.stack((client_appointments['ClientID'], client_appointments['CumulativePercentage']), axis=-1)
#     )
...
#     Find the Client IDs that make up the top 20% of activity count
    ...
    # Create a Pareto chart
    fig7 = px.bar(client_appointments, x='ClientID', y='TotalCountOfEventPerReferralID', title='Pareto Analysis of Client ID Activity')
    
    # Add a line for the cumulative percentage
    fig7.add_trace(go.Scatter(
        x=client_appointments['ClientID'],
        y=client_appointments['CumulativePercentage'],
        mode='lines+markers',
        name='Cumulative Percentage',
        yaxis='y2'
    ...
    # Add hover information for the cumulative percentage line
    fig7.update_traces(
        hovertemplate='<b>Client ID:</b> %{x}<br><b>Appointments:</b> %{y}<br><b>Cumulative Percentage:</b> %{y:.2f}%',
        selector=dict(type='scatter')
    )
    
    # Highlight the top 20% clients
    fig7.add_trace(go.Scatter(
        x=top_20_clients.index + 1,
        y=top_20_clients['CumulativePercentage'],
        mode='markers',
